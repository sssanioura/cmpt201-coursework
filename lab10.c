...
#define handle_error(msg)                                                      \
  ...
  1. How is the client sending data to the server? What protocol?              \
    A.                                                                         \
    The client is sending data using TCP protocol using a socket and then      \
  send() and write().                                                          \
  2. What data is the client sending to the server?                            \
    A.                                                                         \
    A list of random words (Hello, green, apple...).                           \
                                                                               \
  Understanding the Server:                                                    \
  1. Explain the argument that the `run_acceptor` thread is passed as an       \
  argument. A. Recieves a pointer to struct acceptor_args it contains run,                   \
  list_handle and list_lock.                                                   \
  2. How are received messages stored?                                         \
    A.                                                                         \
    Using linked list. where each message is a list_node containng next and    \
  data. The struct tracks last and count.                                                               \
                                                                               \
  3. What does `main()` do with the received messages?                         \
    A.                                                                         \
    Creates acceptor thread, waits for messages, singals shutdown, join s      \
  acceptor thread, callect messages and then checks results. 4.How are threads                                                             \
  used in this sample code? A. There are main threads, acceptor and client                                   \
  threads which handle individual clients.                                     \
                                                                               \
                                                                               \
    Socket q's::                                                               \
                                                                               \
    A.                                                                         \
    Sockets such as server and client socket are usually made non-blocking for \
  hte purpose of handling multiple client connections or avoid deadlocks.                               \
  */
  ...
  while (aargs->run) {
    if (num_clients < MAX_CLIENTS) {
      ...
      if (cfd == -1) {
        ...
        pthread_create(therads[num_clients], NULL, run_client,
...
int main() {
  ...
  while (MAX_CLIENTS * NUM_MSG_PER_CLIENT >)
